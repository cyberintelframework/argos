...
if (! -e "$rrddir") { `mkdir $rrddir`; }
if (! -e "$imgdir") { `mkdir $imgdir`; }
...
sub ProcessHDD {
 ...
 # print "$hdd\n";
  ...
  # if rrdtool database doesn't exist, create it
  if (! -e "$rrddir/$partition_name.rrd")	{
    print "creating rrd database for $partition ...\n";
    RRDs::create "$rrddir/$partition_name.rrd",
  ...
  # insert values into rrd
  RRDs::update "$rrddir/$partition_name.rrd",
...
#  print "swap usage use_rate:$swap_use_rate%, available:${swap_avail} bytes, used:${swap_used} bytes\n";
  ...
  # if rrdtool database doesn't exist, create it
  if (! -e "$rrddir/memory.rrd")	{
    print "creating rrd database for memory ...\n";
    RRDs::create "$rrddir/memory.rrd",
...
sub CreateGraph {
  ...
  if ( $_[3] eq "cpu" ) {
	...
	RRDs::graph "$imgdir/$pngfilename",
		...
		"-u 100",
		"--rigid",
		"DEF:a=$rrddir/$_[0].rrd:user:AVERAGE",
		"DEF:b=$rrddir/$_[0].rrd:system:AVERAGE",
		"DEF:c=$rrddir/$_[0].rrd:iowait:AVERAGE",
		"DEF:d=$rrddir/$_[0].rrd:idle:AVERAGE",
		"CDEF:all=a,b,c,d,+,+,+",
  ...
  if ( $_[3] eq "hdd" ) {
	...
	RRDs::graph "$imgdir/$pngfilename",
		...
		"-v Available(Byte)",
		"--rigid",
		"DEF:a=$rrddir/$partition_name.rrd:use_rate:AVERAGE",
		"DEF:b=$rrddir/$partition_name.rrd:available:AVERAGE",
		"DEF:c=$rrddir/$partition_name.rrd:used:AVERAGE",
		"CDEF:all=a,b,c,+,+",
		"CDEF:unknown=all,UN,INF,UNKN,IF",
  ...
  if ( $_[3] eq "memory" ) {
	...
	RRDs::graph "$imgdir/$pngfilename",
		...
		"-v Usage(Byte)",
		"--rigid",
		"DEF:a=$rrddir/memory.rrd:memory_use_rate:AVERAGE",
		"DEF:b=$rrddir/memory.rrd:memory_available:AVERAGE",
		"DEF:c=$rrddir/memory.rrd:memory_used:AVERAGE",
		"DEF:d=$rrddir/memory.rrd:swap_use_rate:AVERAGE",
		"DEF:e=$rrddir/memory.rrd:swap_available:AVERAGE",
		"DEF:f=$rrddir/memory.rrd:swap_used:AVERAGE",
  ...
  if ( $_[3] eq "traffic" ) {
	...
	RRDs::graph "$imgdir/$pngfilename",
		...
		"-a", "PNG",
		"-v bytes/sec",
		"DEF:in=$rrddir/$_[0].rrd:in:AVERAGE",
		"DEF:out=$rrddir/$_[0].rrd:out:AVERAGE",
